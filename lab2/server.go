...
func main() {
	...
	// WaitGroup для отслеживания всех обработок
	var wg sync.WaitGroup

	// Цикл приёма соединений в отдельной goroutine, чтобы мы могли ловить сигнал
	go func() {
		for {
			conn, err := listener.Accept()
			if err != nil {
				// listener закрыт → выходим
				break
			}
	...
	// Закрываем listener, чтобы Accept() вернул ошибку
	listener.Close()

	// Ждём, пока все активные goroutine завершат обработку
	wg.Wait()
